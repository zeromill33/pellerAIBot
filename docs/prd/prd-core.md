...
### A6. 端到端 E2E 回归测试（持续补齐）

- 以真实 Polymarket event URL 为输入，跑通端到端流水线，用于回归与可观测性验证  
- 覆盖当前已实现的流水线步骤（例如已到 `liquidity.proxy.step` 就覆盖到该步为止）  
- 每个步骤必须产出可读的输入/输出记录，便于定位失败原因与阶段性结果  
- 后续每个 story 推进到新步骤时，必须同步补齐 E2E 覆盖范围，确保全链路完整性持续增长  
...
10) 端到端 E2E 回归测试（真实 URL，逐步输入/输出可见，随 story 持续补齐）  
