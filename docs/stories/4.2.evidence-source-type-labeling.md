...
## Tasks / Subtasks
- [ ] Task 1：在 EvidenceBuilder 中实现来源类型标注规则（AC: 1-6）  
  [Source: prd/appendices/evidence-builder.md#E3. 来源类型映射（必须五选一）]
  - [ ] Subtask 1.1：为 Tavily 结果按 lane + 域名规则赋值 `source_type`
  - [ ] Subtask 1.2：为市场行为类证据赋值 `source_type=市场行为` 并绑定 market url
  - [ ] Subtask 1.3：对未命中白名单的域名明确采用默认规则并保持五选一
- [ ] Task 2：维护官方/主流媒体域名白名单（或黑名单）  
  [Source: prd/appendices/evidence-builder.md#E3. 来源类型映射（必须五选一）]
  - [ ] Subtask 2.1：在配置或常量中声明域名列表并提供可扩展入口
- [ ] Task 3：补充来源类型标注测试覆盖（AC: 1-7）  
  [Source: architecture/tech-stack.md#9) 测试策略]  
  [Source: architecture/coding-standards.md#11.3 单元测试范围（必须覆盖“各种情况”）]
  - [ ] Subtask 3.1：单测覆盖 A/B/C/D 车道与域名映射规则
  - [ ] Subtask 3.2：单测覆盖市场行为证据的 source_type 赋值
  - [ ] Subtask 3.3：单测覆盖白名单/黑名单缺失或未命中时的默认规则
