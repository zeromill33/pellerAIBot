...
## Tasks / Subtasks
- [ ] 新增或扩展 Polymarket Pricing Provider，封装价格相关接口（AC: 1, 2, 3）  
  - [ ] 参考官方文档实现 Get market price / Get midpoint price / Get price history for a traded token  
  - [ ] 建议新增 `src/providers/polymarket/pricing.ts`（或等价文件）  
  [Source: architecture/providers/polymarket-pricing.md#J.3 关键接口（本 MVP 会用到）]
- [ ] 定义价格信息 DTO 与 `PriceSignals`（latest/midpoint/history + 派生指标），并在 orchestrator 层可被消费（AC: 4, 5）  
  [Source: architecture/architecture-core.md#3.2 核心数据结构（内部标准化 DTO）]
- [ ] 依据附录 J 公式实现 PriceSignals，并保留 `history_24h` 原始序列（AC: 4, 5）  
  [Source: architecture/providers/polymarket-pricing.md#J.4.1 PriceSignals 计算口径（必须）]
- [ ] 设定 tokenId 选择规则并记录选用的 tokenId（AC: 8）  
- [ ] 历史价格不足时返回可解释的结构化提示（AC: 9）  
- [ ] 将 `price_context` 合并到 `MarketContext` 并在 pipeline 中可用（AC: 6）  
  [Source: architecture/architecture-core.md#4.1 主流程（单事件）]
- [ ] 更新 LLM 输入字段说明与 prompt（AC: 7）  
  [Source: prd/appendices/prompts.md#G1. Report v1 生成 Prompt（输出必须纯 JSON）]
- [ ] 更新 SQLite 迁移脚本，为 Event 表新增 price_* 字段（AC: 5）  
  [Source: architecture/architecture-core.md#7.2 数据表（SQLite 逻辑模型）]
- [ ] 按 24h/1h 参数拉取历史价格，必要时重采样（AC: 10）  
- [ ] 按 Provider 规范实现超时、重试、退避与缓存（AC: 11）  
...
- [ ] 补充 Provider 级测试（200/429/字段缺失/历史不足/重采样）与 fixture（AC: 9, 10, 11）  
