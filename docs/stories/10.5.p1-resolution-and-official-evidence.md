...
## Tasks / Subtasks
- [x] Task 1：实现 resolution 结构化解析（AC: 1）  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]
  - [x] Subtask 1.1：新增 `src/orchestrator/steps/resolution.parse.step.ts` 并写入解析结果到 ctx
  - [x] Subtask 1.2：更新 `src/validator/schema/report_v1.schema.json`，在 `context` 下新增 `resolution_structured` 字段
  - [x] Subtask 1.3：更新 `prompts/report_v1_generate.prompt.txt` 与 LLM 后处理以输出 `context.resolution_structured`
  - [x] Subtask 1.4：解析失败时返回可审计错误原因
- [x] Task 2：实现官方 resolver 抓取（AC: 2）  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]
  - [x] Subtask 2.1：新增 `src/orchestrator/steps/official.fetch.step.ts` 抓取并生成 `official_sources[]`
  - [x] Subtask 2.2：更新 schema/prompt/后处理，确保 `official_sources` 进入 Report v1 JSON
  - [x] Subtask 2.3：抓取失败需返回明确错误与降级策略（不虚构证据）
- [x] Task 3：Validator 新规则（AC: 3）  
  [Source: architecture/report_quality_P0_P1_implementation.md#P1-3 Resolution Rules 结构化 + 官方 resolver 抓取]
  - [x] Subtask 3.1：在 `src/validator/gates.ts` 增加 `official_sources` 或 `resolution_structured` 必须存在的规则
  - [x] Subtask 3.2：在 `src/orchestrator/errors.ts` 新增 `VALIDATOR_MISSING_OFFICIAL_SOURCE`
- [x] Task 4：测试覆盖（AC: 4）  
  [Source: architecture/coding-standards.md#11.3 单元测试范围（必须覆盖“各种情况”)]
  - [x] Subtask 4.1：解析字段的成功/失败用例
  - [x] Subtask 4.2：official fetch 失败路径与 validator 规则覆盖
...
| 2026-01-30 | v0.2    | 解析/抓取官方来源并新增验证与测试 | James (Dev) |
...
- 新增 resolution.parse/official.fetch 步骤并接入 pipeline。
- 为 Report v1 增加 resolution_structured/official_sources，并在 LLM 输入与后处理校验中同步。
- Validator 增加官方来源存在性闸门，补充解析/抓取/验证测试。

### File List
- src/orchestrator/steps/resolution.parse.step.ts
- src/orchestrator/steps/official.fetch.step.ts
...
- src/validator/gates.ts
- src/validator/schema/report_v1.schema.json
- prompts/report_v1_generate.prompt.txt
- tests/orchestrator/resolution-parse.step.test.ts
- tests/orchestrator/official-fetch.step.test.ts
- tests/orchestrator/report-generate.step.test.ts
- tests/orchestrator/supplement-search.test.ts
- tests/orchestrator/publish-strategy.test.ts
- tests/validator/report-schema.test.ts
- tests/validator/content-gates.test.ts
