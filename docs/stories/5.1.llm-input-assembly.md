...
## Tasks / Subtasks
- [ ] Task 1：定义 LLM 输入映射与字段对齐规则（AC: 1-5）  
  [Source: architecture/architecture-core.md#3.2 核心数据结构（内部标准化 DTO）]  
  [Source: architecture/providers/llm.md#E.3 最小接口]  
  [Source: architecture/coding-standards.md#5.2 证据（Evidence）必须带来源与归因]
  - [ ] Subtask 1.1：从 Orchestrator ctx 提取 `MarketContext` 与 `PriceContext.signals` 并映射到 `market_context`
  - [ ] Subtask 1.2：从 `ClobSnapshot` 映射 `clob_snapshot` 必需字段
  - [ ] Subtask 1.3：将 Tavily 多车道结果或 EvidenceCandidate 映射为 `tavily_results`/EvidenceDigest，保留 URL/来源/时间等可追溯字段
  - [ ] Subtask 1.4：确认 `generateReportV1` 输入结构与 `config.aiProbabilityScale` 设置
- [ ] Task 2：在 Report 生成 step 落地输入组装（AC: 1-6）  
  ...
  - [ ] Subtask 2.1：在 `src/orchestrator/steps/report.generate.step.ts` 组装并传入 LLM 输入
  - [ ] Subtask 2.2：在 `src/providers/llm/prompt.ts`/`index.ts` 中拼装 prompt 输入并加载 `prompts/report_v1_generate.prompt.txt`
  - [ ] Subtask 2.3：对证据/检索结果做稳定排序或去重，保证输入确定性（建议排序：lane → published_at → domain → url）
- [ ] Task 3：补充输入组装测试（AC: 1-6）  
  ...
  [Source: architecture/coding-standards.md#11.8 可重复性（Determinism）要求]
  - [ ] Subtask 3.1：单测覆盖 `market_context/clob_snapshot/tavily_results` 字段完整性与映射正确性
  - [ ] Subtask 3.2：LLM 输入组装测试必须 mock LLM，避免真实调用
  - [ ] Subtask 3.3：对稳定排序/确定性输出添加快照或等价断言
- [ ] Task 4：E2E 回归覆盖到 `report.generate`（AC: 7）  
  [Source: tests/e2e/publish-pipeline.e2e.test.ts]
  - [ ] Subtask 4.1：更新 E2E 默认 stop step 为 `report.generate`（如步骤已就绪）  
  - [ ] Subtask 4.2：确保摘要输出包含 `report.generate` 的 input/output keys
