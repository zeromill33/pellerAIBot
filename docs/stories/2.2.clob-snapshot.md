...
## Tasks / Subtasks
- [ ] 实现或完善 `MarketProvider.getOrderBookSummary(tokenId)`，调用 `/book?token_id=...`（AC: 1, 4）  
  - [ ] 只取前 N 档（N=10）并计算 `best_bid/best_ask/spread/mid`  
  - [ ] 处理空盘口或缺 bids/asks 的情况  
  [Source: architecture/providers/polymarket-clob.md#B.3 关键端点：Get order book summary]
- [ ] 映射 `ClobSnapshot` 并计算 notable walls（AC: 2, 3）  
  - [ ] 规则：size > 均值×k（k=5）  
  [Source: architecture/providers/polymarket-clob.md#B.3 关键端点：Get order book summary]  
  [Source: architecture/providers/polymarket-clob.md#B.4 “大额挂单墙”与“鲸鱼动向”口径（Week 1 现实版）]
- [ ] 按 Provider 通用规范实现超时、重试、退避与短 TTL 缓存（AC: 5）  
  - [ ] 对 429/5xx/网络错误重试，最多 3 次  
  - [ ] 订单簿缓存 TTL=15–60s（可配置）  
...
- [ ] 补充 Provider 级测试（200/429/字段缺失/空盘口）与 fixture（AC: 4, 5）  
