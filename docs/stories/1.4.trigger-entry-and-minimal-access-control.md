...
## Tasks / Subtasks
- [x] Task 1：实现 Admin Bot /publish 入口（AC: 1, 3）[Source: architecture/architecture-core.md#1.3 运营触发入口（推荐方案）] [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]
  - [x] Subtask 1.1：在 `src/bot/index.ts` 注册命令路由并调用 orchestrator
  - [x] Subtask 1.2：在 `src/bot/commands/publish.ts` 解析参数并输出统一回执
- [x] Task 2：实现白名单校验（AC: 2）[Source: architecture/architecture-core.md#1.3 运营触发入口（推荐方案）]
  - [x] Subtask 2.1：在 `src/bot/guard.ts` 校验 admin_user_ids
  - [x] Subtask 2.2：未授权用户返回结构化错误回执
- [x] Task 3：配置加载与校验（AC: 4）[Source: architecture/coding-standards.md#7.1 配置加载与校验（启动期失败）]
  - [x] Subtask 3.1：在 `src/config/config.schema.ts` 校验 admin_user_ids 必填
  - [x] Subtask 3.2：启动期缺失即 fail-fast
- [x] Task 4：补充测试用例（AC: 1-4）[Source: architecture/coding-standards.md#11 测试与质量门禁（必须更严格：build 通过 + 单测覆盖 + 外部依赖集成测试）]
  - [x] Subtask 4.1：单元测试覆盖白名单通过/拒绝
  - [x] Subtask 4.2：集成测试覆盖 /publish 入口调用 orchestrator（mock）
...
| 2026-01-23 | 0.2     | 完成入口与白名单校验 + 配置校验 + 测试补齐 | James |
...
- 已实现 /publish 入口路由与白名单校验，未授权返回结构化错误回执。
- 已新增配置校验与加载工具，缺失 admin_user_ids 启动即失败。
- 已补齐单元/集成测试并通过 `npm test` 与 `npm run build`。
...
- src/config/load.ts
- tests/guard.test.ts
- tests/bot-entry.integration.test.ts
...
入口层边界清晰，权限校验与回执格式化职责分离；配置校验在启动期完成，Telegram SDK 接入与命令路由实现简洁可维护。

### Refactoring Performed
无。

### Compliance Check
- Coding Standards: ✓（入口层仅做解析/权限/回执，未侵入业务链路）
- Project Structure: ✓（bot/config 位置与分层符合约定）
- Testing Strategy: ✓（补齐白名单与入口路由测试）
- All ACs Met: ✓

### Improvements Checklist
- [ ] 可选：为 Telegram bot 添加基于 mock 的启动/路由烟测（避免依赖真实 token）

### Security Review
白名单校验与 bot token 启动校验已实现，未发现新增安全风险。
...
### Gate Status
Gate: PASS → docs/qa/gates/1.4-trigger-entry-and-minimal-access-control.yml  
Risk profile: docs/qa/assessments/1.4-risk-20260123.md  
NFR assessment: docs/qa/assessments/1.4-nfr-20260123.md
