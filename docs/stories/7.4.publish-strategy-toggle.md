...
## Tasks / Subtasks
- [ ] Task 1：配置发布策略开关（AC: 1）  
  [Source: prd/appendices/ops-sop.md#6) 发布策略（配置化二选一）]
  - [ ] Subtask 1.1：在 `src/config/defaults.ts` 与 `config.schema.ts` 增加 `publish_strategy`（auto/approve）
- [ ] Task 2：在 pipeline 中按策略执行发布（AC: 2-4）  
  [Source: architecture/architecture-core.md#4.1 主流程（单事件）]
  - [ ] Subtask 2.1：自动发布：`telegram.publish.step.ts` 直接推送并写回 message_id/status
  - [ ] Subtask 2.2：运营确认：跳过 publish step，保留草稿状态并等待后续命令触发发布
- [ ] Task 3：补充发布策略测试（AC: 1-4）  
  [Source: architecture/tech-stack.md#9) 测试策略]
  - [ ] Subtask 3.1：配置 auto 模式下发布路径
  - [ ] Subtask 3.2：配置 approve 模式下不发布并记录状态
