...
## Tasks / Subtasks
- [x] Task 1：新增 Validator 校验与错误码（AC: 1, 2）  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]  
  [Source: architecture/tech-stack.md#7) 校验与安全护栏（Validator）]
  - [x] Subtask 1.1：在 `src/validator/gates.ts` 增加 Δ 一致性校验与占位符范围白名单
  - [x] Subtask 1.2：在 `src/orchestrator/errors.ts` 新增 `VALIDATOR_METRICS_MISMATCH` 与 `VALIDATOR_PLACEHOLDER_OUTPUT`
  - [x] Subtask 1.3：在 `src/validator/index.ts` 保持闸门顺序稳定，首个失败即返回
- [x] Task 2：补搜触发码收口（AC: 3）  
  [Source: architecture/architecture-core.md#4.3 “补搜”策略（节省额度 + 提升通过率）]  
  [Source: architecture/coding-standards.md#2.2 Step 统一接口（强约束）]
  - [x] Subtask 2.1：在 `src/orchestrator/pipeline.ts` 的补搜触发集合中加入新错误码
  - [x] Subtask 2.2：确保补搜只触发一次，失败后落地 blocked 并返回 validator 原因
- [x] Task 3：新增单元测试覆盖（AC: 4）  
  [Source: architecture/coding-standards.md#11.3 单元测试范围（必须覆盖“各种情况”)]
  - [x] Subtask 3.1：在 `tests/validator/` 增加 delta 一致性校验的通过/失败用例
  - [x] Subtask 3.2：增加占位符范围校验用例（允许范围/禁止范围）
...
| 2026-01-30 | v0.2    | 实现质量闸门补强与测试覆盖 | James (Dev) |
...
- 新增 Δ 一致性与占位符范围校验并扩展补搜触发码。
- 调整测试样例时间字段并补充校验用例；补充补搜在 stopStepId=report.validate 时中断逻辑。
