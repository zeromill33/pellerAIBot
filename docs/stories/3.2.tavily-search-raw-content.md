...
## Tasks / Subtasks
- [x] Task 1：实现/更新 `search.tavily` step 执行 A/B/C 多车道检索（AC: 1, 3, 4）  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]  
  [Source: architecture/coding-standards.md#2.1 Step 文件命名与顺序编排]
  - [x] Subtask 1.1：读取 Query Plan 的 A/B/C 查询并按车道执行（D 车道不启用）
  - [x] Subtask 1.2：将结果映射为 `TavilyLaneResult`（含 `raw_content`）并写入 ctx
  - [x] Subtask 1.3：如需，更新 `src/orchestrator/types.ts` 中 tavily 结果 DTO
- [x] Task 2：落地 Tavily Provider 调用、限流与缓存（AC: 2, 5, 6）  
  [Source: architecture/providers/tavily.md#C.1 Base URL 与鉴权]  
  [Source: architecture/coding-standards.md#3.2 超时 / 重试 / 退避（硬性要求）]  
  [Source: architecture/architecture-core.md#8.2 Tavily 全局速率]  
  [Source: architecture/architecture-core.md#8.3 缓存键策略（与 PRD 附录 A 一致）]
  - [x] Subtask 2.1：在 Tavily client 中实现 `POST /search` 与鉴权、超时、可重试策略
  - [x] Subtask 2.2：实现缓存键 `event_slug + day + lane + query_hash` 与 TTL=1d，并记录 cache_hit
  - [x] Subtask 2.3：对 429/5xx/timeout 统一为 `AppError` 并标记 retryable
- [x] Task 3：补充结构化日志与指标字段（AC: 7）  
  [Source: architecture/coding-standards.md#9.1 结构化日志（必须）]
  - [x] Subtask 3.1：在 step 级别输出结构化日志（含 provider、latency、cache_hit、rate_limited）
- [x] Task 4：补充测试覆盖 Tavily provider 与 step 行为（AC: 1-7）  
  [Source: architecture/tech-stack.md#9) 测试策略]  
  [Source: architecture/coding-standards.md#11.4 Provider 集成测试（外部依赖：必须有“可回放”测试）]
  - [x] Subtask 4.1：使用 fixtures 覆盖 A/B/C 查询结果映射与 `raw_content` 保留
  - [x] Subtask 4.2：覆盖缓存命中与限流行为
  - [x] Subtask 4.3：覆盖 429/5xx/timeout 的 `AppError` 与 retryable 标记
...
| 2026-01-26 | 0.2     | 实现 Tavily 检索 step 与 provider、缓存限流及测试 | James |
| 2026-01-26 | 0.3     | 复用 Tavily provider 以保持缓存与限流 | James |
...
- 已实现 Tavily provider/client/cache 与限流策略，并补充 AppError 处理。
- 已新增 `search.tavily` step 与 pipeline 接入，输出结构化日志。
- 已补充 Tavily fixtures 与 provider/step 测试覆盖。
- 已将 Tavily provider 默认路径改为进程内复用。
...
- src/providers/tavily/cache.ts
- src/providers/tavily/client.ts
- src/providers/tavily/index.ts
- tests/config.test.ts
- tests/fixtures/tavily/lane-a.json
