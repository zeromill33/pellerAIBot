...
## Tasks / Subtasks
- [ ] Task 1：实现批处理编排与并发控制（AC: 1, 2）[Source: architecture/architecture-core.md#4.2 多事件批处理] [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]
  - [ ] Subtask 1.1：在 `src/orchestrator/batch.ts` 接收 URL/slug 列表并设置并发度（配置化）
  - [ ] Subtask 1.2：为每条链接生成 run_id 并独立执行 pipeline（失败不阻断批次）
- [ ] Task 2：定义批处理结果结构与回执汇总（AC: 2, 3）[Source: architecture/architecture-core.md#4.2 多事件批处理] [Source: architecture/coding-standards.md#6.1 统一 AppError]
  - [ ] Subtask 2.1：在 `src/orchestrator/types.ts` 定义批处理结果 DTO（success/fail 列表）
  - [ ] Subtask 2.2：在 `src/bot/commands/publish.ts` 组装并输出汇总回执（含错误码与提示）
- [ ] Task 3：补充错误码与结构化日志字段（AC: 2, 4）[Source: architecture/coding-standards.md#6.2 错误码命名规范] [Source: architecture/coding-standards.md#9.1 结构化日志（必须）]
  - [ ] Subtask 3.1：新增/复用批处理相关错误码（如 `ORCH_BATCH_*`）
  - [ ] Subtask 3.2：批次级日志包含 request_id，单条包含 run_id 与 event_slug
- [ ] Task 4：补充测试用例（AC: 1-4）[Source: architecture/coding-standards.md#11 测试与质量门禁（必须更严格：build 通过 + 单测覆盖 + 外部依赖集成测试）]
  - [ ] Subtask 4.1：单元测试覆盖并发度配置、单条失败不影响其他条目
  - [ ] Subtask 4.2：集成测试覆盖 /publish 批处理的成功/失败汇总回执（mock providers）
