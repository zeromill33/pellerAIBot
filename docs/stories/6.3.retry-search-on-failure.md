...
## Tasks / Subtasks
- [x] Task 1：扩展 Validator 错误输出以支持补搜决策（AC: 1,5）  
  [Source: architecture/architecture-core.md#5.2 输出错误格式（用于运营可读回执）]
  - [x] Subtask 1.1：对证据不足类错误返回 `suggestion: { action: "ADD_SEARCH", preferred_lane: "C" }`
  - [x] Subtask 1.2：在 error code 中区分“证据不足/来源不足/失败路径不足”类原因
- [x] Task 2：在 Orchestrator 中实现补搜重跑（AC: 1-4）  
  [Source: architecture/architecture-core.md#4.3 “补搜”策略（节省额度 + 提升通过率）]  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]
  - [x] Subtask 2.1：在 `src/orchestrator/pipeline.ts` 或 `query.plan.build.step.ts` 支持补搜 lane 升级（C basic→advanced）
  - [x] Subtask 2.2：当建议启用 D 时，生成 2–3 类不同类型 Query 并执行 Tavily 检索
  - [x] Subtask 2.3：补搜后重跑 EvidenceBuilder → LLM → Validator，并限制补搜次数（1 次）
- [x] Task 3：补充补搜流程测试（AC: 1-4）  
  [Source: architecture/coding-standards.md#11.3 单元测试范围（必须覆盖“各种情况”)]
  - [x] Subtask 3.1：模拟证据不足触发补搜并验证通过
  - [x] Subtask 3.2：模拟额度不足/补搜后仍失败并返回建议
...
| 2026-01-28 | 0.3     | 实现补搜与重跑验证流程 | James |
...
- `npx vitest run tests/orchestrator/supplement-search.test.ts`

### Completion Notes List
- 证据不足类闸门输出 ADD_SEARCH suggestion，并用专用错误码区分来源不足/失败路径不足。
- report.validate 失败时触发补搜：升级 C 车道、按需启用 D 查询组，并限制为 1 次。
- 补搜后重跑搜索、证据构建、LLM 与 Validator；补搜超额时返回明确限额错误。
- 新增补搜流程单测覆盖成功与限额失败路径。
