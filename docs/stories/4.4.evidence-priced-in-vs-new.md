...
## Tasks / Subtasks
- [x] Task 1：在 EvidenceBuilder 中实现 novelty 标注规则（AC: 1-5）  
  [Source: prd/appendices/evidence-builder.md#E5. priced-in vs new（Week 1 规则）]
  - [x] Subtask 1.1：根据 `published_at` 与时效关键词判定 `new`
  - [x] Subtask 1.2：结合去重聚合的重复转述数量判定 `priced`
  - [x] Subtask 1.3：基于 `price_context.signals.change_24h` 与阈值判定 `new/priced`
  - [x] Subtask 1.4：无判定时设置 `unknown`
- [x] Task 2：落地 novelty 阈值配置与校验（AC: 2-4）  
  [Source: architecture/coding-standards.md#7.1 配置加载与校验（启动期失败）]
  - [x] Subtask 2.1：在 `config.defaults.ts` 与 `config.schema.ts` 中补齐阈值默认值与校验
- [x] Task 3：补充 novelty 标注测试覆盖（AC: 1-5）  
  ...
  - [x] Subtask 3.1：单测覆盖 48h/72h 边界与时效关键词判定
  - [x] Subtask 3.2：单测覆盖重复转述 ≥3 与价格稳定/波动阈值判定
  - [x] Subtask 3.3：单测覆盖 `unknown` 默认路径
...
| 2026-01-27 | 0.3     | 实现 novelty 规则、配置与测试 | James |
| 2026-01-27 | 0.4     | 修复类型报错并通过 build | James |
| 2026-01-27 | 0.5     | 修复 novelty 判定边界与关键词误判 | James |
| 2026-01-27 | 0.6     | Tavily D 车道默认改为 always | James |
| 2026-01-27 | 0.7     | 更新配置文档并回归 E2E | James |
| 2026-01-27 | 0.8     | E2E 摘要按来源排序并输出全量证据 | James |
| 2026-01-27 | 0.9     | E2E 摘要过滤重复证据 | James |
...
- 实现证据新旧判定：时间窗/时效关键词/重复来源/价格变动阈值。
- 新增 novelty 配置默认值与校验。
- 补充并通过相关单测覆盖。
- 修复配置输入类型与测试用例类型问题，`pnpm -s build` 通过。
- 运行 `pnpm -s test -- tests/orchestrator/query-plan.step.test.ts tests/orchestrator/pipeline-query-plan.step.test.ts tests/providers/tavily/lanes.test.ts`。
- 运行 E2E（live，含 Tavily）：默认摘要输出 `tests/e2e/tmp/e2e-step-summary.json`，D 车道摘要输出 `tests/e2e/tmp/e2e-step-summary-dlane.json`。
- E2E 摘要增强 `evidence.build` 统计与样例输出，并重新生成摘要文件。
- 修复 change_24h 选择逻辑，避免忽略可用信号；时效关键词按词边界匹配，避免子串误判。
- 运行 `pnpm -s test -- tests/orchestrator/evidence-build.step.test.ts`。
- Tavily D 车道默认改为 always，相关单测使用显式 conditional 配置避免默认影响。
- 更新配置与 Tavily 文档默认值，运行 live E2E 验证 D 车道。
- E2E 摘要中 `evidence.build` 证据按来源优先级排序并输出全量列表。
- E2E 摘要中过滤 `repeated=true` 证据并重新生成。
...
- docs/prd/appendices/config.md
- docs/prd/appendices/tavily-query-generator.md
- tests/e2e/publish-pipeline.e2e.test.ts
...
实现符合分层与可测性要求，novelty 规则具备可配置阈值与确定性测试覆盖；对 change_24h 可用性与关键词误判的修复提升了准确性与鲁棒性。

### Refactoring Performed
无（本次仅评审与验证）。

### Compliance Check
- Coding Standards: ✓（分层与纯函数约束符合）
- Project Structure: ✓（文件位置与命名符合）
- Testing Strategy: ✓（单测 + live E2E 覆盖关键路径）
- All ACs Met: ✓

### Improvements Checklist
- [x] 复核 novelty 逻辑与边界条件覆盖
- [x] 验证阈值配置可覆盖与默认值正确
- [x] 运行关键单测与 live E2E
- [ ] （可选）评估 D 车道默认 always 的额度成本监控方案

### Security Review
未发现新增安全风险；无鉴权/敏感数据处理变化。

### Performance Considerations
E2E 摘要输出全量证据仅用于测试，不影响生产路径；线上调用不受影响。

### Files Modified During Review
- docs/stories/4.4.evidence-priced-in-vs-new.md
- docs/qa/gates/4.4-evidence-priced-in-vs-new.yml

### Gate Status
Gate: PASS → docs/qa/gates/4.4-evidence-priced-in-vs-new.yml
Risk profile: docs/qa/assessments/4.4-risk-20260127.md
NFR assessment: docs/qa/assessments/4.4-nfr-20260127.md
