...
## Tasks / Subtasks
- [x] Task 1：实现 Tavily 结果过滤 step（AC: 1, 2）  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]  
  [Source: architecture/coding-standards.md#2.1 Step 文件命名与顺序编排]
  - [x] Subtask 1.1：新增 `src/orchestrator/steps/tavily.verify.step.ts`（或同等命名）实现过滤规则
  - [x] Subtask 1.2：输出 `tavily_results_filtered` 与 `dropped_evidence[]`（url + reason）供审计
- [x] Task 2：接线主流程并保证 LLM 使用过滤结果（AC: 3）  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]  
  [Source: architecture/architecture-core.md#4.1 主流程（单事件）]
  - [x] Subtask 2.1：在 `src/orchestrator/pipeline.ts` 中将过滤 step 插入 `search.tavily` 与 `evidence.build/report.generate` 之间
  - [x] Subtask 2.2：确保 `evidence.build` 与 `report.generate` 使用过滤后的 `tavily_results`
  - [x] Subtask 2.3：过滤后不足触发可补搜错误码/提示（优先 C lane advanced）
- [x] Task 3：新增单元测试覆盖（AC: 4）  
  [Source: architecture/coding-standards.md#11.3 单元测试范围（必须覆盖“各种情况”)]
  - [x] Subtask 3.1：覆盖命中/不命中关键词的过滤分支
  - [x] Subtask 3.2：覆盖缺少 `raw_content` 的过滤分支
  - [x] Subtask 3.3：覆盖时效性优先级分支
...
| 2026-01-30 | v0.2    | 实现相关性过滤 step 与测试覆盖 | James (Dev) |
...
- 新增 Tavily 相关性过滤 step，输出过滤结果与审计丢弃清单。
- Pipeline 接入过滤结果与补搜触发，并新增单测覆盖分支。

### File List
- src/orchestrator/steps/tavily.verify.step.ts
- src/orchestrator/pipeline.ts
- src/orchestrator/errors.ts
- src/orchestrator/types.ts
- tests/orchestrator/tavily-verify.step.test.ts
