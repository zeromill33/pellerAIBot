...
## Tasks / Subtasks
- [x] Task 1：实现 `getLatestReport(slug)` 存储查询（AC: 2）  
  [Source: architecture/architecture-core.md#3.1 Provider 接口清单（概览）]
  - [x] Subtask 1.1：在 `src/storage/sqlite/report.repo.ts` 查询最新 report（按生成时间或 id 排序）
- [x] Task 2：实现 `/status <slug>` 命令与回执（AC: 1,3,4）  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]
  - [x] Subtask 2.1：在 `src/bot/commands/status.ts` 调用 `triggerStatus/getLatestReport`
  - [x] Subtask 2.2：在 `src/orchestrator/index.ts` 增加 `triggerStatus` 入口（如尚未实现）
  - [x] Subtask 2.3：未找到时返回标准化错误
- [x] Task 3：补充 `/status` 测试（AC: 1-4）  
  [Source: architecture/tech-stack.md#9) 测试策略]
  - [x] Subtask 3.1：模拟存在 report 与不存在 report 的两种回执
...
- 新增 getLatestReport 存储查询与 triggerStatus 入口。
- 增加 /status 命令与回执格式，未找到时返回标准化错误。
- 补充 status 命令与存储查询测试。
...
- tests/telegram-bot.smoke.test.ts
