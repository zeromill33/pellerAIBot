...
## Tasks / Subtasks
- [x] Task 1：实现 StorageAdapter 三表写入（AC: 1-5）  
  [Source: architecture/providers/storage.md#F.1 Week 1 推荐：SQLite（本地）]
  - [x] Subtask 1.1：在 `src/storage/sqlite/event.repo.ts` upsert Event 字段
  - [x] Subtask 1.2：在 `src/storage/sqlite/evidence.repo.ts` 批量写入 Evidence 字段
  - [x] Subtask 1.3：在 `src/storage/sqlite/report.repo.ts` 保存 Report（含 status/validator_code/message）
- [x] Task 2：在 pipeline 中落地存储步骤（AC: 1,4）  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]
  - [x] Subtask 2.1：在 `src/orchestrator/steps/persist.step.ts` 调用 StorageAdapter 保存三表
  - [x] Subtask 2.2：Validator 失败时也落地报告（status=blocked + reason）
- [x] Task 3：补充存储落地测试（AC: 1-5）  
  [Source: architecture/tech-stack.md#9) 测试策略]
  - [x] Subtask 3.1：使用 sqlite test db 验证三表写入与字段映射
  - [x] Subtask 3.2：覆盖 validator 失败与 success 的落地路径
...
- 新增 SQLite StorageAdapter（migrations + 三表 repo），支持事务写入三表。
- Pipeline 新增 persist step，并在 validator 失败时记录 blocked 报告与原因。
- 增加三表落地测试（success 与 blocked 路径）。

### File List
- migrations/001_init.sql
- migrations/002_indexes.sql
- src/orchestrator/pipeline.ts
- src/orchestrator/steps/persist.step.ts
- src/orchestrator/errors.ts
- src/storage/index.ts
- src/storage/sqlite/db.ts
- src/storage/sqlite/migrations.ts
- src/storage/sqlite/event.repo.ts
- src/storage/sqlite/evidence.repo.ts
- src/storage/sqlite/report.repo.ts
- src/storage/sqlite/index.ts
- tests/storage/persist.step.test.ts
- package.json
- package-lock.json
