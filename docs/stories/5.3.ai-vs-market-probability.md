...
## Tasks / Subtasks
- [ ] Task 1：在 LLM Prompt/输出中强化 AI vs 市场概率段落（AC: 1-5）  
  [Source: prd/appendices/prompts.md#G1. Report v1 生成 Prompt]  
  [Source: prd/appendices/report-v1-schema.md#附录 B：Report v1 JSON Schema（0–100 概率）]
  - [ ] Subtask 1.1：确保 prompt 明确要求 `ai_vs_market` 字段与 0–100 概率口径
  - [ ] Subtask 1.2：确保 prompt 明确禁止下注建议/资金管理建议（drivers 不得含“喊单”措辞）
- [ ] Task 2：在 LLMProvider 输出后保证字段完整与合规（AC: 1-5）  
  [Source: architecture/providers/llm.md#E.1 输出强约束]  
  [Source: architecture/coding-standards.md#4.3 反幻觉策略（必须落地）]
  - [ ] Subtask 2.1：在 `postprocess`/调用侧校验 `ai_vs_market` 字段存在、drivers 长度 1–3
  - [ ] Subtask 2.2：若出现“喊单”措辞，确保 Validator 阻断并返回明确失败原因
- [ ] Task 3：补充 AI vs 市场概率输出测试（AC: 1-5）  
  ...
  - [ ] Subtask 3.1：Mock LLM 输出，覆盖 drivers 过多/为空、概率越界等失败分支
  - [ ] Subtask 3.2：验证 limitations.not_included 包含固定免责声明
- [ ] Task 4：E2E fake 输出补齐 AI vs 市场概率字段（AC: 6）  
  [Source: tests/e2e/publish-pipeline.e2e.test.ts]
  - [ ] Subtask 4.1：更新 E2E mock LLM 输出，包含 `ai_vs_market` 与免责声明字段
