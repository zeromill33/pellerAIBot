...
## Tasks / Subtasks
- [x] Task 1：实现内容闸门规则与错误码映射（AC: 1-8）  
  [Source: prd/appendices/validator.md#F2. 内容闸门（失败则阻断发布）]  
  [Source: architecture/coding-standards.md#6.1 统一 AppError]
  - [x] Subtask 1.1：在 `src/validator/gates.ts` 实现各项规则检查
  - [x] Subtask 1.2：为每条规则配置明确 `VALIDATOR_*` 错误码与可读 message/suggestion
  - [x] Subtask 1.3：黑名单词表可配置/可扩展（至少含 buy/long/short/梭哈/重仓/建议下注）
- [x] Task 2：在 Validator 入口聚合闸门结果（AC: 1-8）  
  [Source: architecture/source-tree.md#1) 源码目录结构（AI Signal Bot）]
  - [x] Subtask 2.1：`validateReport()` 在 schema 通过后执行内容闸门
  - [x] Subtask 2.2：首个失败即返回（避免多重冲突），保留 `suggestion` 以提示补搜
- [x] Task 3：补充内容闸门测试（AC: 1-8）  
  [Source: architecture/coding-standards.md#11.3 单元测试范围（必须覆盖“各种情况”)]
  - [x] Subtask 3.1：覆盖每条闸门规则的失败/通过用例
  - [x] Subtask 3.2：覆盖黑名单触发与 drivers 违规的阻断路径
...
| 2026-01-28 | 0.3     | 实现内容闸门与测试 | James |
...
- `npx vitest run tests/validator/report-schema.test.ts tests/validator/content-gates.test.ts`

### Completion Notes List
- 新增内容闸门规则与可配置黑名单，按规则返回 VALIDATOR_* 错误码与建议。
- Validator 在 schema 通过后执行闸门校验，首个失败即返回。
- 补充内容闸门测试覆盖每条规则与 drivers 违规路径。
